...
line llong(line a, point inter) {
    if (dcmp((a.t - a.s).len() + (inter - a.t).len(), (inter - a.s).len())) {
        return line(a.s, inter);
    } else if (dcmp((a.t - inter).len() + (a.s - a.t).len(),
                    (a.s - inter).len())) {
        return line(inter, a.t);
    }
    return a;
}
signed main(void) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (parallel(str, tmp)) {
            ...
            double ang;
            if (tmp.ang < -eps)
                ang = tmp.ang + asin(sqrt(y));
            else
                ang = tmp.ang - asin(sqrt(y));
            add.x = 1, add.y = 1 / tan(ang);
    ...
    li[0] = li[tot];
    // for (int i = 1; i <= tot; i++) {
    //     point tmp = intersection(li[i], li[i + 1]);
    //     li[i] = llong(li[i], tmp);
    //     tmp = intersection(li[i], li[i - 1]);
    //     li[i] = llong(li[i], tmp);
    // }
    for (int i = 1; i <= tot; i++)
        dbg(li[i].s.x, li[i].s.y, li[i].t.x, li[i].t.y, li[i].ang);
